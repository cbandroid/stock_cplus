...
FundFlow::FundFlow(GlobalVar *&pGlobalVar)
{
...
//    fundFlowChart->setAttribute(Qt::WA_DeleteOnClose);
    ...
    fundFlowChart->setGeometry(850-200, 150, 1000, 800);
    fundFlowChart->hide();
...
void FundFlow::getEastPlateFundFlow(int days)
{
    ...
    FundFlowCol<<"名称X"<<h+"涨跌幅"<<"主力净额"<<"主力净占比"<<"超大单净额"<<"超大单净占比"<<"大单净额"
    ...
    /*std::sort(FundFlowList.begin(),FundFlowList.end(),[](QStringList a,QStringList b){
        return  m_pGlobalVar->is_asc[1] ? a[1].toFloat()>b[1].toFloat():a[1].toFloat()<b[1].toFloat();
    });*/
    bool is_asc=m_pGlobalVar->is_asc[1];
    sortByStrColumn(&FundFlowList,m_pGlobalVar->curSortNum[1],is_asc);
    ...
    for (int i=0;i<FundFlowList.count();++i)
    {
        for (int j=0;j<FundFlowList.at(i).count();++j)
        {
            ...
            {
                s=FundFlowList.at(i)[j];
                model->setItem(i,j,new QStandardItem(format_conversion(s.toFloat())));
                if (s.left(1)=="-")
...
void FundFlow::drawFundFlowChart(QPainter *painter)
{
    ...
    painter->drawText(width/2,textHeight,format_conversion(maxMinKChart[0]));
    painter->drawText(width/2,(height-bottom)/2,format_conversion(maxMinKChart[1]));
    painter->drawLine(0,height-40,width/2,height-40);
    painter->drawLine(width/2,height-365,width/2,height-40);

    painter->drawText(width/2,height-345,format_conversion(maxMinHKChart[0]));
    painter->drawText(width/2,height-40,format_conversion(maxMinHKChart[1]));
    ...
    for (int j=1;j<6;++j)
    {
        ...
        {
            p1=(maxMinKChart[0]-fundFlowKList.at(i-1)[j].toFloat())/interval*(height-bottom)/2;
            p2=(maxMinKChart[0]-fundFlowKList.at(i)[j].toFloat())/interval*(height-bottom)/2;
    ...
    for (int i=0;i<4;++i)
    {
        painter->drawText(x[i],y[i],na[i]+format_conversion(pieData[i]));
    ...
    for (int j=1;j<6;++j)
    {
        ...
        {
            p1=(2*maxMinHKChart[0]-maxMinHKChart[1]-fundFlowHKList.at(i-1)[j].toFloat())/interval*(height-bottom)/2;
            p2=(2*maxMinHKChart[0]-maxMinHKChart[1]-fundFlowHKList.at(i)[j].toFloat())/interval*(height-bottom)/2;
            painter->setPen(QPen(fiveColor[j-1],2));
    ...
    int nCount=fundFlowHKList.count();
    for (int i=nCount-twentyN;i<nCount;++i)
    {
    ...
    for (int i=0;i<5;++i)
    {
        five=fiveTotal[i]-min;
        twenty=twentyTotal[i]-min;
    ...
    if (isClick)
    {
        ...
        for (int i=0;i<5;++i)
        {
            ...
            textFund[i]->setText(format_conversion(fiveTotal[i]));
        }
        isClick=false;
    }
}
...
void FundFlow::getBoardStock(QList<StockInfo> *&pTableList,QString name)
{
    NetworkManager networkManager;
    QByteArray allData;
    ...
    if (file.open(QFile::ReadOnly))
    {
        ...
        for (int i=0;i<data.count();++i)
        {
            ...
            if (name==strLine[1])
            {
                 qUrl="http://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=2000&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&wbp2u=|0|0|0|web&fid=f3&fs=b:"+strLine[0]+"+f:!50&fields=f2,f3,f5,f6,f8,f9,f12,f14,f15,f16,f17,f18,f20,f21,f24,f25,f22&_=1665552114833";
...
void FundFlow::getIntervalHighLow()
{
    ...
    QByteArray allData;
     ...
     QString qUrl= "https://legulegu.com/stockdata/member-ship/get-high-low-statistics/all";
     NetworkManager networkManager;
     allData = networkManager.getSync<QByteArray>(QUrl(qUrl));
...
void FundFlow::drawIntervalHighLowChart(QPainter *painter)
{
    ...
    for (int j=0;j<6;++j)
    {
        ...
        if (isShow[j])
        {
            for (int i=1;i<maxNums;++i)
            {
                p1=(1-model->item(maxNums-i,j+2)->text().toFloat()/4000)*tableChart->height();
                p2=(1-model->item(maxNums-i-1,j+2)->text().toFloat()/4000)*tableChart->height();
...
QColor whichColor( QStringList *pMyStockCode,QString code)
{
    for (int i=0;i<pMyStockCode->count();++i)
    {
        if (code==pMyStockCode->at(i))
            return QColor(255,140,0);
    }
    return QColor(72,61,139);
}

void FundFlow::getStockPoolStrong( QStringList *pMyStockCode,QString date)
{
    ...
    QString qUrl ="https://push2ex.eastmoney.com/getTopicQSPool/get?ut=7eea3edcaed734bea9cbfc24409ed989&dpt=wz.ztzt&Pageindex=0&pagesize=170&sort=zdp:desc&date="+date+"&_=1621590489736";
    NetworkManager networkManager;
    allData = networkManager.getSync<QByteArray>(QUrl(qUrl));
    ...
    if (jsonError.error == QJsonParseError::NoError)
    {
        ...
        for (int i = 0; i < nSize; ++i)
        {
            ...
            n=t.toInt();
            s=t.toString()+"/"+ceilMap.value("zttj").toJsonValue().toVariant().toMap().value("ct").toString();
            code=ceilMap.value("c").toString();
            ...
            model->item(i,1)->setForeground(whichColor(pMyStockCode,code));
            ...
            a=ceilMap.value("amount").toFloat();
            model->setItem(i,5,new QStandardItem(format_conversion(a)));
            if (a>= 1000000000)
                model->item(i,5)->setForeground(QColor(153, 0, 153));
            else if (a>=300000000)
                model->item(i,5)->setForeground(QColor(0, 191, 255));
            model->setItem(i,6,new QStandardItem(format_conversion(ceilMap.value("ltsz").toFloat())));
            model->setItem(i,7,new QStandardItem(format_conversion(ceilMap.value("tshare").toFloat())));
...
void FundFlow::getNorthFundFlow(QStringList *pMyStockCode,QString days)
{
    QStringList northFundFlowCol;
    northFundFlowCol<<"代码"<<"名称"<<"收盘价"<<"涨跌幅"<<"持股数"<<"持市值"<<"持占流通股比"<<"持占总股本比"
                   <<"增股数"<<"增市值"<<"增市值增幅"<<"增占流通股比"<<"增占总股本比"<<"所属板块";
    QString curDate=m_pGlobalVar->curRecentWorkDay(2).date().toString("yyyy-MM-dd");
    QByteArray allData;

    QString qUrl ="https://datacenter-web.eastmoney.com/api/data/v1/get?sortColumns=ADD_MARKET_CAP&sortTypes=-1&pageSize=5000&pageNumber=1&reportName=RPT_MUTUAL_STOCK_NORTHSTA&columns=ALL&source=WEB&client=WEB&filter=(TRADE_DATE%3D%27"+curDate+"%27)(INTERVAL_TYPE%3D%22"+days+"%22)";
    NetworkManager networkManager;
    allData = networkManager.getSync<QByteArray>(QUrl(qUrl));
    ...
    if (jsonError.error == QJsonParseError::NoError)
    {
        ...
        for (int i = 0; i < nSize; ++i)
        {
            value = data.at(i);
            ceilMap = value.toVariant().toMap();
            code=ceilMap.value("SECURITY_CODE").toString();
            model->setItem(i,0,new QStandardItem(code));
            ...
            model->item(i,1)->setFont(boldFont);
            model->item(i,1)->setForeground(whichColor(pMyStockCode,code));
            s=ceilMap.value("CHANGE_RATE").toString();
...
void FundFlow::getDragonTigerList(QStringList *pMyStockCode,int nums,int pages)
{
    ...
    QString StartDate;
    QString endDate;
    QDateTime t,curDate=QDateTime::currentDateTime();
    int counts=0;
    for (int i=0;i<60;++i)
    {
        t=curDate.addDays(-i);
        if (m_pGlobalVar->isZhWorkDay(t,1))
        {
            counts+=1;
            if (counts==1)
    ...
    for (int j=1;j<pages+1;++j)
    {
        ...
        if (jsonError.error == QJsonParseError::NoError)
        {
            ...
            data=jsonObject.value("result").toObject().value("data").toArray();
            n=model->rowCount();
            nSize=data.size();
            for (int i = 0; i < nSize; ++i)
            {
                ...
                ceilMap = value.toVariant().toMap();
                code=ceilMap.value("SECURITY_CODE").toString();
                model->setItem(i+n,0,new QStandardItem(code));
                ...
                c=ceilMap.value("CHANGE_RATE").toFloat();
                model->setItem(i+n,4,new QStandardItem(format_conversion(c)));
                if (c<0)
                {
                ...
                model->setItem(i+n,5,new QStandardItem(format_conversion(c)));
                if (c<0)
                ...
                model->setItem(i+n,6,new QStandardItem(format_conversion(ceilMap.value("BILLBOARD_BUY_AMT").toFloat())));
                model->item(i+n,6)->setForeground(Qt::red);
                model->setItem(i+n,7,new QStandardItem(format_conversion(ceilMap.value("BILLBOARD_SELL_AMT").toFloat())));
                model->item(i+n,7)->setForeground(QColor(0, 191, 0));
                model->setItem(i+n,8,new QStandardItem(format_conversion(ceilMap.value("BILLBOARD_DEAL_AMT").toFloat())));
                model->setItem(i+n,9,new QStandardItem(format_conversion(ceilMap.value("ACCUM_AMOUNT").toFloat())));
                model->setItem(i+n,10,new QStandardItem(format_conversion(ceilMap.value("DEAL_NET_RATIO").toFloat())));
                model->setItem(i+n,11,new QStandardItem(format_conversion(ceilMap.value("DEAL_AMOUNT_RATIO").toFloat())));
                float a=ceilMap.value("TURNOVERRATE").toFloat();
                model->setItem(i+n,12,new QStandardItem(format_conversion(a)));
                if (a>=15)
                    model->item(i+n,12)->setForeground(QColor(204, 204, 0));
                model->setItem(i+n,13,new QStandardItem(format_conversion(ceilMap.value("FREE_MARKET_CAP").toFloat())));
...
void FundFlow::countSingleStockBoard(QStringList *pMyStockCode,QString nums,int pages)
{
    ...
    {
        qUrl ="https://datacenter-web.eastmoney.com/api/data/v1/get?sortColumns=BILLBOARD_TIMES%2CLATEST_TDATE%2CSECURITY_CODE&sortTypes=-1%2C-1%2C1&pageSize=1000&pageNumber="+QString::number(i)+"&reportName=RPT_BILLBOARD_TRADEALLNEW&columns=ALL&source=WEB&client=WEB&filter=(STATISTICS_CYCLE%3D%22"+nums+"%22)";
        allData = networkManager.getSync<QByteArray>(QUrl(qUrl));
        ...
        if (jsonError.error == QJsonParseError::NoError)
        {
            ...
            for (int i = 0; i <nSize; ++i)
            {
                ...
                QVariantMap ceilMap = value.toVariant().toMap();
                QString code=ceilMap.value("SECURITY_CODE").toString();
                model->setItem(i+n,0,new QStandardItem(code));
                model->setItem(i+n,1,new QStandardItem(ceilMap.value("SECURITY_NAME_ABBR").toString()));
                ...
                model->setItem(i+n,6,new QStandardItem(format_conversion(b)));
                if (b>0)
                    model->item(i+n,6)->setForeground(Qt::red);
                else if (b<0)
                    model->item(i+n,6)->setForeground(QColor(0, 191, 0));
                model->setItem(i+n,7,new QStandardItem(format_conversion(ceilMap.value("BILLBOARD_BUY_AMT").toFloat())));
                model->item(i+n,7)->setForeground(Qt::red);
                model->setItem(i+n,8,new QStandardItem(format_conversion(ceilMap.value("BILLBOARD_SELL_AMT").toFloat())));
                model->item(i+n,8)->setForeground(QColor(0, 191, 0));
                model->setItem(i+n,9,new QStandardItem(format_conversion(ceilMap.value("BILLBOARD_DEAL_AMT").toFloat())));
                ...
                model->setItem(i+n,12,new QStandardItem(format_conversion(b)));
                if (b>0)
                    model->item(i+n,12)->setForeground(Qt::red);
                else if (b<0)
                    model->item(i+n,12)->setForeground(QColor(0, 191, 0));
                model->setItem(i+n,13,new QStandardItem(format_conversion(ceilMap.value("ORG_BUY_AMT").toFloat())));
                model->item(i+n,13)->setForeground(Qt::red);
                model->setItem(i+n,14,new QStandardItem(format_conversion(ceilMap.value("ORG_SELL_AMT").toFloat())));
                model->item(i+n,14)->setForeground(QColor(0, 191, 0));

                for (int j=0;j<4;++j)
                {
                    b=ceilMap.value(name[j]).toFloat();
                    model->setItem(i+n,j+15,new QStandardItem(format_conversion(b)));
                    if (b>=100)
...
void FundFlow::getStockHot(QStringList *pMyStockCode,QList<StockInfo> *pTableListCopy)
{
    QJsonObject json;
    ...
    QString qUrl="https://emappdata.eastmoney.com/stockrank/getAllCurrentList";
    NetworkManager networkManager;
    networkManager.TryPostExSync(QUrl(qUrl),dataArray,allData );
    ...
    if (jsonError.error == QJsonParseError::NoError)
    {
        ...
        for (int i = 0; i < nSize; ++i)
        {
            value = data.at(i);
            ceilMap = value.toVariant().toMap();
            code=ceilMap.value("sc").toString().right(6);
            rank=ceilMap.value("rk").toString();
            l = 0;
            r = n - 1;
            while (l <= r)
            {
                mid = (l + r) / 2;
                if (pTableListCopy->at(mid).code == code)
                {
                    model->setItem(i,0,new QStandardItem(pTableListCopy->at(mid).code));
                    model->setItem(i,1,new QStandardItem(pTableListCopy->at(mid).name));
                    model->setItem(i,2,new QStandardItem(QString::number(pTableListCopy->at(mid).close)));
                    float p=pTableListCopy->at(mid).pctChg;
                ...
                else if (pTableListCopy->at(mid).code > code)
                    r = mid - 1;
                else
                    l = mid + 1;
            }
...
void FundFlow::openFundRank(QString ft,QString sc)
{
    ...
    QString qUrl = "https://fund.eastmoney.com/data/rankhandler.aspx?op=ph&dt=kf&ft="+ft+"&rs=&gs=0&sc="+sc+"&st=desc&sd="+sd+"&ed="+ed+"&qdii=&tabSubtype=,,,,,&pi=1&pn=20000&dx=1&v=0.1591891419018292";
    QByteArray allData;
    NetworkManager networkManager;
    bool bResult = networkManager.TryGetSync(qUrl, request,allData );
    if (not bResult)
        return;
...
void FundFlow::getRoyalFlushFundFlow()
{
    ...
    QFile scriptFile(m_pGlobalVar->currentPath+fileName);
...
//    request.setRawHeader("X-Requested-With", "XMLHttpRequest");
    ...
    for (int n=0;n<len;++n)
    {
        for (int i=1;i<=page[n];++i)
        {
            ...
            qUrl =url[n]+QString::number(i)+"/ajax/1/free/1/";
            int nCode = networkManager.TryGetSync(QUrl(qUrl),request,allData);//,FundFlow::handleReadyRead);
            if (nCode !=200)
                continue;
            QString html=QTextCodec::codecForName("GBK")->toUnicode(allData);
            QString str=m_pGlobalVar->peelStr(html,"<tbody","-1");
            ...
            while(1)
            {
                if (str.indexOf("<tr")==-1)
                    break;
                pair=m_pGlobalVar->cutStr(str,"<tr","</tr>");
                str=pair.second;
                QStringList l;
                m_pGlobalVar->getAllContent(pair.first,l,"<td");
//                QString p;
                for (int j=0;j<10;++j)
                {
//                    if (j==2)
...
void FundFlow::getNotNormalStock(QStringList *pMyStockCode)
{
//    getHisTimeShareTick();
    QStringList NotNormalStock;
    NotNormalStock<<"代码"<<"名称"<<"时间"<<"类型"<<"相关信息";
    QByteArray allData;
    QString qUrl="https://push2ex.eastmoney.com/getAllStockChanges?type=8201,8202,8193,4,32,64,8207,8209,8211,8213,8215,8204,8203,8194,8,16,128,8208,8210,8212,8214,8216&ut=7eea3edcaed734bea9cbfc24409ed989&pageindex=0&pagesize=10000&dpt=wzchanges&_=1692089577464";
    ...
    if (jsonError.error == QJsonParseError::NoError)
    {
        ...
        for (int i = 0; i < nSize; ++i)
        {
            ...
            ceilMap = value.toVariant().toMap();
//            QStringList list;
            cod=ceilMap.value("c").toString();
            model->setItem(i,0,new QStandardItem(cod));
            model->setItem(i,1,new QStandardItem(ceilMap.value("n").toString()));
            model->item(i,1)->setFont(boldFont);
            model->item(i,1)->setForeground(whichColor(pMyStockCode,cod));
...
void FundFlow::getHisTimeShareTick()
{
   // QNetworkRequest request;
    ...
    while(1)
    {
        ...
        qUrl="https://market.finance.sina.com.cn/transHis.php?symbol=sz301205&date=2022-11-28&page="+QString::number(page);
        ...
        while(1)
        {
            if (str.indexOf("<tr")==-1)
                break;
            pair=m_pGlobalVar->cutStr(str,"<tr","</tr");
            s=m_pGlobalVar->peelStr(pair.first,"<tr","-1");
            QStringList l;
            QStringList list;
            m_pGlobalVar->getAllContent(s,l,"<t");
...
void FundFlow::getTimeShareMin(QList<timeShareChartInfo> *&pHisTimeShareChartList,QString code,QString date)
{
    QJSEngine myEngine;
    QString fileName = "/list/sf_sdk.js";
    QFile scriptFile(m_pGlobalVar->currentPath+fileName);

    pHisTimeShareChartList->clear();

    if (!scriptFile.open(QIODevice::ReadOnly)){
      //  qDebug() <<     m_pGlobalVar->currentPath+fileName;
        return;
    }
    QTextStream stream(&scriptFile);
    QString contents = stream.readAll();
    ...
    QByteArray allData;
//    QNetworkAccessManager naManager =QNetworkAccessManager();
    QString qUrl="https://finance.sina.com.cn/realstock/company/"+code+"/hisdata/"+date.left(4)+"/"+date.mid(5,2)+".js?d="+date;
  ...
  //  nCount=1;  // for test
    ...
    {
        args << ss[i];
        funcValue=func.call(args);

       // qDebug()<< funcValue.toString();

        //date1=funcValue.property(0).property("date").toString();
        rcvDateTime=funcValue.property(0).property("date").toDateTime();//.toUTC();
        //qDebug()<<date1;
        rcvDate=rcvDateTime.addSecs(28800).toString("yyyy-MM-dd");
        if (date!=rcvDate)
        {
           // qDebug() <<date <<"not found ! "<<rcvDate;
            args.clear();
            continue;
        }
        l=funcValue.property("length").toInt();
        m_pGlobalVar->hisPreClose=funcValue.property(0).property("prevclose").toNumber();

        curTime=QDateTime::fromString("2023-06-05 09:25", "yyyy-MM-dd hh:mm"); //？
        ...
        for (int j=1;j<l;++j)
        {
          ...
          pHisTimeShareChartList->append(info);
        }
        args.clear();

       // m_pGlobalVar->mHisTimeShareChartList=hisTimeShareChartList;
//        for (int i=0;i<m_pGlobalVar->mHisTimeShareChartList.count();++i)
//            qDebug()<<m_pGlobalVar->mHisTimeShareChartList.at(i).price<<m_pGlobalVar->mHisTimeShareChartList.at(i).vol;
    }

   // qDebug() <<"getTimeShareMin " << high1 <<","<< low1;
}
...
void FundFlow::initAllNews()
{
    if (isGetNews)
        return;
    isGetNews=true;
    m_pGlobalVar->annoucementList.clear();
    if (m_pGlobalVar->curCode.left(1)!="1" and m_pGlobalVar->curCode.left(3)!="399" and m_pGlobalVar->curCode.length()!=5)
    {
        getAnnoucement();
        getGreatEvent();
//        getNews();
    ...
    QStringList list;
    list<<""<<"[结束]"<<"(2000-01-01)";
    m_pGlobalVar->annoucementList.append(list);
    std::sort(m_pGlobalVar->annoucementList.begin(),m_pGlobalVar->annoucementList.end(),[](QStringList a,QStringList b){
        return a[2]>b[2];
    });
    isGetNews=false;
    //    for (int i=0;i<m_pGlobalVar->annoucementList.count();++i)
    //        qDebug()<<m_pGlobalVar->annoucementList.at(i)[2];
}
...
void FundFlow::getAnnoucement()
{
    ...
    QString url="http://ddx.gubit.cn/gonggao/"+m_pGlobalVar->curCode;
   ...
   while(1)
   {
       if (str.indexOf("<tr")==-1)
           break;
       QPair<QString, QString> pair=m_pGlobalVar->cutStr(str,"<tr","</tr");
       QString s=m_pGlobalVar->peelStr(pair.first,"<tr","-1");
       QStringList list;
       QStringList l;
       QString href=m_pGlobalVar->getAttributeContent(s,"href","\"");
       m_pGlobalVar->getAllContent(s,l,"<td");
       if (l.size()>2)
       {
           list.clear();
...
void FundFlow::getEastAnnoucement()
{
    ...
    QString url="https://np-anotice-stock.eastmoney.com/api/security/ann?sr=-1&page_size=50&page_index=1&ann_type=A&client_source=web&stock_list="+m_pGlobalVar->curCode+"&f_node=0&s_node=0";
    ...
    if (jsonError.error == QJsonParseError::NoError)
    {
        ...
        for (int i = 0; i < nSize; ++i)
        {
            ...
            l<<ceilMap.value("title").toString()<<"[东方公告]"<<"("+ceilMap.value("notice_date").toString()
              <<"https://data.eastmoney.com/notices/detail/"+m_pGlobalVar->curCode+"/"+ceilMap.value("art_code").toString()+".html";
            m_pGlobalVar->annoucementList.append(l);
        }
    }
}
...
{
    QString s=m_pGlobalVar->getCookies("http://www.stcn.com/article/search.html");
...
//    QString url1="https://search-api-web.eastmoney.com/search/jsonp?cb=&param=%7B%22uid%22%3A%227111416627128474%22%2C%22keyword%22%3A%22%E4%B8%AD%E9%99%85%E6%97%AD%E5%88%9B%22%2C%22type%22%3A%5B%22cmsArticleWebOld%22%5D%2C%22client%22%3A%22web%22%2C%22clientType%22%3A%22web%22%2C%22clientVersion%22%3A%22curr%22%2C%22param%22%3A%7B%22cmsArticleWebOld%22%3A%7B%22searchScope%22%3A%22default%22%2C%22sort%22%3A%22default%22%2C%22pageIndex%22%3A1%2C%22pageSize%22%3A50%2C%22preTag%22%3A%22%3Cem%3E%22%2C%22postTag%22%3A%22%3C%2Fem%3E%22%7D%7D%7D&_=1686979606619";
    ...
    for (int i=1;i<2;++i)
    {
//        QString url="https://search.cnstock.com/search/result/"+QString::number(i)+"?t=0&k="+m_pGlobalVar->curName.left(m_pGlobalVar->curName.indexOf("("));
        url="http://www.stcn.com/article/search.html?search_type=news&keyword="+m_pGlobalVar->curCode+"&page_time="+QString::number(i);
        ...
        QString str=m_pGlobalVar->peelStr(html,"<ul class=\"list infinite-list\"","-1");
        QString href;
        QString s,t;
        QPair<QString, QString> pair;
        while(1)
        {
            if (str.indexOf("<li class=")==-1)
                break;
            pair=m_pGlobalVar->cutStr(str,"<li","</li");
            s=m_pGlobalVar->peelStr(pair.first,"<li","-1");
            QStringList list;
            QStringList l;
            href=m_pGlobalVar->getAttributeContent(s,"href","\\\"");
            m_pGlobalVar->getAllContent(s,l,"<a");
            m_pGlobalVar->getAllContent(s,l,"<span");
            t=l[l.count()-1];
            if (t.contains("\\n"))
                t=m_pGlobalVar->annoucementList.back()[2];
            else if (t.length()<13)
...
//        QString str=m_pGlobalVar->peelStr(html,"<div class=\"result-left\"","-1");
//        while(1)
//        {
//            if (str.indexOf("<div class=\"result-article\"")==-1)
//                break;
//            QPair<QString, QString> pair=m_pGlobalVar->cutStr(str,"<div","</div");
//            QString s=m_pGlobalVar->peelStr(pair.first,"<div","-1");
//            QStringList list;
//            QStringList l;
//            m_pGlobalVar->getAllContent(s,l,"<a");
//            m_pGlobalVar->getAllContent(s,l,"<span");
...
{
    QString name=m_pGlobalVar->curName.left(m_pGlobalVar->curName.indexOf("(")).left(m_pGlobalVar->curName.indexOf("-"));
