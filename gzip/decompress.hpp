...
#pragma GCC diagnostic pop
        ...
        inflate_s.next_in = reinterpret_cast<z_const Bytef*>((unsigned char *)data);
...
int ungzip(Bytef *dest, uLongf *destLen,const Bytef *source, uLong sourceLen)
{
	z_stream inflate_s;
...
#pragma GCC diagnostic pop
    ...
    inflate_s.next_in = reinterpret_cast<z_const Bytef*>((unsigned char *)source);

#ifdef DEBUG
	// Verify if size (long type) input will fit into unsigned int, type used for zlib's
	// avail_in
	std::uint64_t size_64 = sourceLen * 2;
	if (size_64 > (std::numeric_limits<unsigned int>::max)()) {
		inflateEnd(&inflate_s);
		throw std::runtime_error("size arg is too large to fit into unsigned int type x2");
	}
#endif
	//if (sourceLen > max_ || (sourceLen * 2) > max_) {
	//	inflateEnd(&inflate_s);
	//	throw std::runtime_error("size may use more memory than intended when decompressing");
	//}
	inflate_s.avail_in = static_cast<unsigned int>(sourceLen);
	std::size_t size_uncompressed = 0;
	do {
		std::size_t resize_to = size_uncompressed + 2 * sourceLen;
		//if (resize_to > max_) {
		//	inflateEnd(&inflate_s);
		//	throw std::runtime_error(
		//		"size of output string will use more memory then intended when decompressing");
		//}
		//output.resize(resize_to);
		inflate_s.avail_out = static_cast<unsigned int>(2 * sourceLen);
		inflate_s.next_out = reinterpret_cast<Bytef*>(&dest[0] + size_uncompressed);
		int ret = inflate(&inflate_s, Z_FINISH);
		if (ret != Z_STREAM_END && ret != Z_OK && ret != Z_BUF_ERROR) {
			std::string error_msg = inflate_s.msg;
			inflateEnd(&inflate_s);
		...
		size_uncompressed += (2 * sourceLen - inflate_s.avail_out);
	} while (inflate_s.avail_out == 0);
	inflateEnd(&inflate_s);
