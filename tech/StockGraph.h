...
public:
	...
	BOOL        IsRefresh() { return m_bInvalidate; }
	void        SetRefresh(BOOL bInv ) { m_bInvalidate = bInv; }
	...
	void		ClearCurStock( );
	BOOL		SetCurStock( const TCHAR * szCode );
...
protected:
	// Move Operation
	BOOL		MoveTo( int nIndex, BOOL bShiftPressed, BOOL bCtrlPressed, BOOL bKeyState, CPoint point=CPoint(0,0) );
    ...
    void        DrawNotExistStockDataInfo( CDC * pDC ); // add 2008.3.17
	...
	void		DrawBorderMovingLine( CDC * pDC, CPoint point, int nColorIndex, BOOL bAddOrRemove );
	BOOL		DrawSelectionLine( CDC * pDC, CPoint point, BOOL bAddOrRemove );
	void		DrawSelectionRect( CDC * pDC, CPoint ptBegin, CPoint ptEnd, BOOL bAddOrRemove );
	void		DrawKData( CDC * pDC );
public:
...
protected:
	...
	int         DrawBuySell( CDC * pDC, CStockInfo & info,int xStart, int yStart, int nWidth );

	// KLine Technical Attributes
	void		DrawTechMA( CDC * pDC, CRect rect, BOOL bDrawTitle, double dMin, double dMax );
    void		DrawTechEXPMA( CDC * pDC, CRect rect, BOOL bDrawTitle, double dMin, double dMax );
	...
	void        DrawTechSKDJ( CDC * pDC, CRect rect, BOOL bDrawTitle );
	...
	void		DrawTechUtil( CDC * pDC, CRect rect, BOOL bDrawTitle, double dMin, double dMax,
							   int nTech, CTechnique * pTech, CKData * pKData, CSPDWordArray & adwDays ,int nDotCount =2 );
	void		DrawTechUtil1( CDC * pDC, CRect rect, BOOL bDrawTitle, double dMin, double dMax,
							   int nTech, CTechnique * pTech, CKData * pKData, COLORREF clrLine, int nDrawTechType  ,int nDotCount =2);
	void		DrawTechUtil2( CDC * pDC, CRect rect, BOOL bDrawTitle, double dMin, double dMax,
							   ...
							   CString strTitle1, CString strTitle2  ,int nDotCount =2);
	void		DrawTechUtil3( CDC * pDC, CRect rect, BOOL bDrawTitle, double dMin, double dMax,
							   ...
							   CString strTitle1, CString strTitle2, CString strTitle3 ,int nDotCount =2);
	void		DrawTechUtil4( CDC * pDC, CRect rect, BOOL bDrawTitle, double dMin, double dMax,
							   ...
							   CString strTitle1, CString strTitle2, CString strTitle3, CString strTitle4  ,int nDotCount =2);
	void		DrawTechUtil5( CDC * pDC, CRect rect, BOOL bDrawTitle, double dMin, double dMax,
							   ...
							   CString strTitle1, CString strTitle2, CString strTitle3, CString strTitle4, CString strTitle5  ,int nDotCount =2);
...
protected:
	...
	std::vector<float>  m_HighPrice;
	std::vector<float>  m_LowPrice;
 	std::vector<UINT>   m_HighIndex;
	std::vector<UINT>   m_LowIndex;

	BOOL IsHighLabel(int Index);     //某段区间里的最高
	BOOL IsLowLabel(int Index);    //某段区间里的最低
