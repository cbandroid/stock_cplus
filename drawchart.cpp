...
DrawChart::DrawChart(QWidget *parent)  noexcept
    : QWidget{parent}
{
...
//    qDebug()<<volAveHeight;
    ...
    painter->drawText(rect,Qt::AlignLeft, GlobalVar::curCode+" "+GlobalVar::curName);

   if (GlobalVar::mTimeShareChartList.isEmpty())
        return;
    //绘制时间
   int count =GlobalVar::mTimeShareChartList.count();
    QList<timeShareChartInfo>::const_iterator ptr =GlobalVar::mTimeShareChartList.begin();
...
void DrawChart::drawHisTimeShare(QPainter *painter)
{
    ...
    QRect rect;
    painter->setPen(Qt::white);
    QList<timeShareChartInfo>::const_iterator ptr =GlobalVar::mHisTimeShareChartList.begin();
    ...
    for (int i=0;i<trendsTotal;++i)
    {
        ...
        if (i>=1)
        {
            ...
            if ((ptr+1)->price>GlobalVar::hisPreClose)
                painter->setPen(Qt::red);
            else if ((ptr+1)->price<GlobalVar::hisPreClose)
                painter->setPen(QColor(0, 255, 0));
            else
                painter->setPen(Qt::white);
            painter->drawLine(QPointF(WIDTHEDGE+aveWidth*i, timeShareChartHeight-BOTTOMHEIGHTEDGE-vol*volAveHeight), QPointF(WIDTHEDGE+aveWidth*i, timeShareChartHeight-BOTTOMHEIGHTEDGE));
...
void DrawChart::drawCandleChart(QPainter *painter)
{
    ...
    if (end<50 or end>total) // fixed 2024.10.7
        end=total;
...
//        qDebug()<<aveWidth;
  ...
  //  QRect rect;
   ...
   // qDebug() <<"drawchart " << begin <<"," << end <<"," <<GlobalVar::mCandleChartList.size();

    QList<candleChartInfo>::const_iterator ptr = GlobalVar::mCandleChartList.begin();
    static bool changecolor=false;
    for (int nIdx=begin;nIdx<end;++nIdx)
    {
        open= (ptr+nIdx)->open;
        if (nIdx==total-1 and (GlobalVar::baseInfoData[0] > FLT_MIN and GlobalVar::baseInfoData[0] < FLT_MAX) )
        {
            close =GlobalVar::baseInfoData[0];
        }
        else
...
//        if (high!=low)
        ...
        if (nIdx==total-1)
        {
           ...
           rect=QRect(curXPos,(highPoint-close)*aveHeight+KTOPHEIGHTEDGE+5,6, 6); // 圆的位置和大小
           painter->drawEllipse(rect);
           painter->setBrush(oldBrush);
           painter->setPen(oldColor) ;
        }
//        qDebug()<<aveHeightVol<<candleHighLowPoint[2];
        ...
        if (nIdx>0)
        {
            float preMA=(ptr+nIdx-1)->MA5;
            float MA=(ptr+nIdx)->MA5;
            ...
            preMA=(ptr+nIdx-1)->MA10;
            MA=(ptr+nIdx)->MA10;
            painter->setPen(QColor(255, 255, 0));
            if (preMA>0)
                painter->drawLine(QPointF(preXPos,(highPoint-preMA)*aveHeight+KTOPHEIGHTEDGE),
                                 QPointF(curXPos,(highPoint-MA)*aveHeight+KTOPHEIGHTEDGE));
            preMA=(ptr+nIdx-1)->MA20;
            MA=(ptr+nIdx)->MA20;
            painter->setPen(QColor(0, 255, 0));
            if (preMA>0)
                painter->drawLine(QPointF(preXPos,(highPoint-preMA)*aveHeight+KTOPHEIGHTEDGE),
                                 QPointF(curXPos,(highPoint-MA)*aveHeight+KTOPHEIGHTEDGE));
            preMA=(ptr+nIdx-1)->MA60;
            MA=(ptr+nIdx)->MA60;
...
void DrawChart::appendAnnNews(int end)
{
    ...
    int nKdCount =GlobalVar::mCandleChartList.count();
    if ((nKdCount-1)<n)
        n=nKdCount-1;
    if (n<0){
        return;
	...
	int count =GlobalVar::annoucementList.count();
    QString c,l,t,time;
    int result;
    ...
    {    
        ...
        {
            c=GlobalVar::annoucementList.at(i)[0];
            l=GlobalVar::annoucementList.at(i)[1];
